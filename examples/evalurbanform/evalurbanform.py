...
def calculate_srfs_area(occ_srflist):
    ...
    for srf in occ_srflist:
        area = area + pylibudo.py3dmodel.calculate.face_area(srf)
    return area
...
plane_polygon = pylibudo.py3dmodel.construct.make_polygon(points3)

occfaces = []
ext1faces = pylibudo.py3dmodel.fetch.faces_frm_solid(extrude1)
occfaces.extend(ext1faces)
ext2faces = pylibudo.py3dmodel.fetch.faces_frm_solid(extrude2)
occfaces.extend(ext2faces)

fai,fuse_psrfs, projected_faces, windplane, surfaces_projected = pylibudo.urbanformeval.frontal_area_index(occfaces, plane_polygon, (-1,-1,0))
