...
def generate_sensor_surfaces_for_occ_solid(building_solid, xdim, ydim):
    face_list = envuo.py3dmodel.fetch.faces_frm_solid(building_solid)
    ...
    for f in face_list:
        ...
        a_sensor_srfs, a_sensor_pts, a_sensor_dirs = envuo.gml3dmodel.generate_sensor_surfaces(f, xdim, ydim)
...
def building_solar(building_solids, terrain_faces, weatherfilepath, latitude, longtitude, meridian, start_mth, 
    ...
    rad = envuo.py2radiance.Rad(base_file_path, data_folder_path)
    ...
    for tface in terrain_faces:
        tface_pts = envuo.gml3dmodel.pyptlist_frm_occface(tface)
        srfname = "terrain_srf" + str(tf_cnt)
        #just use pure white paint
        srfmat = "RAL9010_pur_white_paint"
        envuo.py2radiance.RadSurface(srfname, tface_pts, srfmat, rad)
        tf_cnt+=1
...
terrain_triangles = envuo.shp2citygml.terrain2d23d_tin(poly_path, "DN")
display2dlist.append(terrain_triangles)
colour_list.append("WHITE")
#display the edges of the terrain
tedges = []
for t in terrain_triangles:
    twire = envuo.py3dmodel.fetch.wires_frm_face(t)[0]
    tedge = envuo.py3dmodel.fetch.edges_frm_wire(twire)
    tedges.extend(tedge)
...
bsolids = envuo.shp2citygml.building2d23d(building_shpfile, "Height_ag", terrain_triangles)
#369 buildings
time3 = time.clock()
...
topo_list, irrad_res = building_solar(bsolids, terrain_triangles, weatherfilepath, latitude, longtitude, meridian, start_mth, 
                                      end_mth, start_day, end_day, start_hour, end_hour, ab, xdim, ydim)
...
envuo.py3dmodel.construct.visualise_falsecolour_topo(irrad_res, topo_list, falsecolour_file, image_file, display2dlist, colour_list)
